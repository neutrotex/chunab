...
const getConstituenciesByDistrict = (provinceId, districtId) => {
    const province = nepalData.find(p => p.id === provinceId);
    if (!province) return [];
...
const getBoothLocationsByConstituency = (constituencyId) => {
...
export const getAllConstituencies = async (req, res) => {
    try {
        // Flatten all constituencies from all provinces and districts
            ...
            for (const district of province.districts) {
                for (const constituency of district.constituencies) {
                    allConstituencies.push({
                        ...
                        boothLocations: constituency.boothLocations
                    });
                }
            }
        }
...
export const getConstituenciesByDistrictId = async (req, res) => {
    try {
        const { provinceId, districtId } = req.params;
        const constituencies = getConstituenciesByDistrict(provinceId, districtId);
...
export const getBoothLocationsByConstituencyId = async (req, res) => {
    try {
        const { constituencyId } = req.params;
        const boothLocations = getBoothLocationsByConstituency(constituencyId);
